...
export default create_n_ary_declarations(
  ...
  max_curry_level - 1,
  args => `
    function $${args.curry_level}arity<${['N extends string', 'V extends Record<N, Function>', ...args.generics].join(',')}>(
      arity: ${args.curry_level},
      method: N
    ): CurriedFunction${args.curry_level + 1}<${[...args.types, 'V', args.return_type].join(',')}>;
  `,
  args => `
    import {${args.curry_levels.map(i => `CurriedFunction${i + 1}`).join(',')}} from './$curried-functions';
