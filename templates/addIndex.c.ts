...
const outer_variables = [...new Array(max_outer_count)].map((_, index) => `x${index + 1}`);
const outer_generics = [...new Array(max_outer_count)].map((_, index) => `P${index + 1}`);
const outer_paramters = outer_variables.map((variable, index) => `${variable}: ${outer_generics[index]}`);
...
export default create_n_ary_declarations(
  1,
  max_curry_level,
  args => outer_arguments.map(outer_args => `
    function $v${args.curry_level}x${outer_args.count}<${[...args.types, 'U', ...outer_args.generics, 'V', args.return_type].join(',')}>(
      iterFn: (fn: (${args.parameters.join(',')}) => U, ${[...outer_args.parameters, 'target: V'].join(',')}) => ${args.return_type}
    ): CurriedFunction${outer_args.count + 2}<(${
      [...args.parameters, 'index: number', 'target: V'].join(',')
    }) => U, ${[...outer_args.generics, 'V', args.return_type].join(',')}>;
  `).join(','),
  args => `
    import {${args.curry_levels.slice(1).map(i => `CurriedFunction${i}`).join(',')}} from './$curried-functions';
