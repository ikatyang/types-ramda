...
export const create_composition_declarations = (
  ...
  const parameter_names = [...new Array(max_parameter_count)].map((_, index) => `v${index + 1}`);
  const parameter_generics = [...new Array(max_parameter_count)].map((_, index) => `V${index + 1}`);

  const declarations = [`declare const ${name}: typeof $;`];
  for (let function_count = 1; function_count <= max_function_count; function_count++) {
    ...
    const current_sorted_function_names = function_names.slice(0, function_count);
    if (kind === 'compose') {
      current_sorted_return_generics.reverse();
      current_sorted_function_names.reverse();
    }

    for (let parameter_count = 1; parameter_count <= max_parameter_count; parameter_count++) {
      ...
      declarations.push(`
        declare function $<${[...current_parameter_generics, ...current_return_generics].join(',')}>(
          ${current_return_generics.map((_, index) => {
