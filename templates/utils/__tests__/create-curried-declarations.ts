...
const parse_type = (type_string: string) =>
  (dts.parse(`type X = ${type_string};`).members[0] as dts.ITypeDeclaration)
    .type as dts.IFunctionType;
const emit = (members: dts.ITopLevelMember[]) =>
  dts.emit(dts.create_top_level_element({ members }));

it('should generate correctly with curried types', () => {
  expect(
    emit(
      create_curried_declarations(
        '$.ts',
